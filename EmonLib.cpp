...
void EnergyMonitor::setVCAL(double _VCAL)
{
  VCAL = _VCAL;
}
...
//   int16_t rawValue = adc.getRawResult();
//   return rawValue;
// }
...
void EnergyMonitor::calcVI(unsigned int crossings, unsigned int timeout)
{
  ...
  {
    // startV = adc.getLastConversionResults();
    if(inPinV == 0) {
      ...
      adc.startADCReading(ADS1X15_REG_CONFIG_MUX_SINGLE_2, true);
      startV = adc.getLastConversionResults();
    } else if (inPinV == 3) {
      adc.startADCReading(ADS1X15_REG_CONFIG_MUX_SINGLE_3, true);
      startV = adc.getLastConversionResults();
    }
  ...
  while ((crossCount < crossings) && ((millis()-start)<timeout))
  {
    ...
    if(inPinV == 0) {
      adc.startADCReading(ADS1X15_REG_CONFIG_MUX_SINGLE_0, true);
      sampleV = adc.getLastConversionResults();
    } else if(inPinV == 1) {
      adc.startADCReading(ADS1X15_REG_CONFIG_MUX_SINGLE_1, true);
      sampleV = adc.getLastConversionResults();
    } else if (inPinV == 2) {
